...
#include "AndroidUdpFdSocket.hpp"

using namespace Pravala;

AndroidUdpFdSocket::AndroidUdpFdSocket ( SocketOwner * owner, int64_t networkId ):
    UdpFdSocket ( owner ),
    _netId ( networkId )
{
}

AndroidUdpFdSocket::~AndroidUdpFdSocket()
{
}

bool AndroidUdpFdSocket::sockInitFd ( SocketApi::SocketType sockType, int & sockFd )
{
    if ( sockFd >= 0 )
        return true;

    if ( !UdpFdSocket::sockInitFd ( sockType, sockFd ) )
        return false;
