...
namespace Pravala
{
/// @brief Config option that holds a single IP address "multi set" list value.
/// "Multi set" list means that every time it is set it will append the new value to the list.
class ConfigIpAddressMultiSetList: public ConfigOpt
{
    public:
        /// @brief Constructor.
        ...
        ConfigIpAddressMultiSetList ( uint8_t flags, const char * cfgName, const String & helpText );
        ...
        /// @param [in] helpText The help text for this option.
        ConfigIpAddressMultiSetList ( const char * cmdLineName, char cmdLineFlag, const String & helpText );
        ...
        inline const List<IpAddress> & value() const
        {
            return _value;
        }
    ...
    protected:
        List<IpAddress> _defaultValue; ///< Default value
        List<IpAddress> _value; ///< Current value
